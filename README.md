...
- Copy the repository to the box and run it. Modules are flat in the repo root; the tools in
  `tools/` bootstrap `sys.path` themselves, so `python tools/corpus_stats.py` works from the
  repo root with no `PYTHONPATH`.
...
[Service]
Type=simple
WorkingDirectory=/opt/melon-chat-corpus
ExecStart=/usr/bin/python3 collector.py --config /opt/melon-chat-corpus/collector.json
Restart=always
RestartSec=5
