...
**Non-advancing-cursor guard:** if a **full** page's last `ts_us` equals the `cursor_us` you sent *and* every row in the page deduplicated away, you have hit a tie group larger than 100 rows. The page-full condition is load-bearing and was originally left implicit here, which is exactly how it came to be omitted from the implementation: **without it the guard fires on the ordinary last fetch of every healthy walk**, because the cursor sits on the last row written and the inclusive lower bound re-delivers precisely that one row — a short page with `written == 0` and `max_ts == cursor_us`. That is end-of-window, not a tie group; nothing lies beyond a short page to be lost. Since `tie_skips` is cumulative, never reset, and wired to a health warning, a single spurious bump pins the collector at `warn` forever and destroys the signal this counter exists to carry. `pageSize` is already at max, so advance `cursor_us += 1`, increment `tie_skips`, and log at WARNING with the timestamp and the potential row loss. (Only reachable in whole-second legacy data — a 2013 sample had 6 tie groups over 100 rows; the 2026 sample had zero **[V]**. A gap-backfilling collector should never see it, so treat any occurrence as a signal that the lookback clamp misfired.)
